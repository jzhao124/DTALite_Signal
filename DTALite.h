...
enum e_signal_representation_model {signal_model_continuous_flow = 0,  signal_model_link_effective_green_time, signal_model_timing_status, signal_model_spatial_queue};
...
public:
	DTANodeMovement()
	{
		...
		movement_hourly_capacity = 100; // the default of hourly movement capacity is 100
...
public:
	DTALink(int TimeSize, double Length, double JamDensity)  // TimeSize's unit: per min
	{
		...
		for(int p=0; p<20; p++)
		{
		m_LeftTurn_SaturationFlowRate_In_vhc_per_hour_per_lane[p] = 1900;
		m_LeftTurnGreenEndTime_In_Second[p] = 0;
		m_LeftTurnGreenStartTime_In_Second[p] = 0;
		m_GreenEndTime_In_Second[p] = 0;
		m_GreenStartTime_In_Second[p] = 0;
		m_SaturationFlowRate_In_vhc_per_hour_per_lane[p] = 1800;


		m_plan_startime_in_sec[p] = -1;
		m_plan_endtime_in_sec[p] = -1;
		m_to_node_cycle_in_sec[p] = 0;
		m_to_node_offset_in_sec[p] = 0;
		}
		...
		int number_of_cells = max(1,Length*JamDensity*m_InflowNumLanes);

		for (int i = 0; i < number_of_cells; i++)
		{
			m_Cell.push_back(-1);
			m_Cell_Prev.push_back(-1);
	...
	std::map<int, int> m_Timing_Status_LeftTurnMap;

	float GetTimeDependentCapacityAtSignalizedIntersectionFromTimingStatus(double CurrentTimeInMin);
	float GetTimeDependentLeftTurnCapacityAtSignalizedIntersectionFromTimingStatus(double CurrentTimeInMin);

	std::vector<int> m_Cell;
	std::vector<int> m_Cell_Prev;

	void UpdateCellVehiclePosition(int current_time_interval, int cell_no, int vehicle_id);
...
public:
	...
	std::vector<CarFollowingData> m_CarFollowingDataVector;  // key is the node id
...
void g_OutputRoutingPolicyData(ofstream &output_PathMOE_file, int DemandLoadingStartTimeInMin, int DemandLoadingEndTimeInMin, int cut_off_volume = 50);
void g_OutputLinkPathProportionData(ofstream &output_PathMOE_file, int DemandLoadingStartTimeInMin, int DemandLoadingEndTimeInMin, int cut_off_volume = 50);
void g_OutputLinkProportionData(ofstream &output_PathMOE_file, int DemandLoadingStartTimeInMin, int DemandLoadingEndTimeInMin, int cut_off_volume = 50);
