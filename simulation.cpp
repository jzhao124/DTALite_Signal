...
#pragma omp parallel for
	for (int li = 0; li< link_size; li++)
	{
		...
		while (pLink->LoadingBuffer.size() >0 && pLink->GetNumberOfLanes(DayNo, CurrentTime) > 0.01)  // no load vehicle into a blocked link
		{
			...
			{
				pLink->LoadingBuffer.pop_front(); // the BPR function that no density constraint is imposed

				// jun's edit
				if ((g_SignalRepresentationFlag == signal_model_timing_status) && pLink->m_LeftTurn_NumberOfLanes >= 1 && pLink->m_LeftTurn_DestNodeNumber == vi.veh_next_node_number)
				{
					pLink->LeftEntrance_Queue.push_back(vi);
					pLink->UpdateCellVehiclePosition(meso_simulation_time_interval_no, 0, vi.veh_id);
...
#pragma omp parallel for
	for (int li = 0; li< link_size; li++)
	{
		...
		if (g_TrafficFlowModelFlag != trm_car_following) // queue based model 
		{
			...
			while (pLink->EntranceQueue.size() >0)  // if there are vehicles in the entrance queue
			{
				...
				if (pLink->m_FromNodeNumber == 1 && pLink->m_ToNodeNumber == 5)
				{
		...
		{
			// copy cell states
			for (int c = 0; c < pLink->m_Cell.size(); c++)
			{
				pLink->m_Cell_Prev[c] = pLink->m_Cell[c];
				pLink->m_Cell[c] = -1;
			...
			for (int c = 0; c < pLink->m_Cell.size(); c++)
			{
				if (pLink->m_Cell_Prev[c] >= 0)  // with vehicle in previous state
				{
					DTAVehicle* pVehicle = g_VehicleMap[pLink->m_Cell_Prev[c]];

					int CanMovePosition = c;

					int K_steps = 5;  // can be determined by free flow speed of vehicle

					for (int i = c + 1; i < min(c + 1 + K_steps, pLink->m_Cell.size()); i++)
					{
						if (pLink->m_Cell_Prev[i] >= 0)  // with car
							break;
						else // without car
						{
							CanMovePosition = i;
					...
					int vehicle_id = pLink->m_Cell_Prev[c];

					pLink->UpdateCellVehiclePosition(meso_simulation_time_interval_no,CanMovePosition, vehicle_id);
				}
			}
			...
			int last_cell_no = pLink->m_Cell.size() - 1;
			if (pLink->m_Cell[last_cell_no] >= 0 /*move a car*/ && pLink->m_Cell_Prev[last_cell_no] <0 /*no car before*/)  // if there are vehicles in the entrance queue
			{
				struc_vehicle_item vi = pLink->EntranceQueue.front();
				DTAVehicle* pVehicle = g_VehicleMap[vi.veh_id];
					pLink->EntranceQueue.pop_front();
					pLink->ExitQueue.push_back(vi);  // entering the exit queue (to be discharged)
	...
	else // queueing model and car following model
	{
...
#pragma omp parallel for
		for (int li = 0; li < link_size; li++)
		{
			...
			if (pLink->m_FromNodeNumber == 2 && pLink->m_ToNodeNumber == 5 && CurrentTime >= 60)
			{
				TRACE("");
			}
			...
			if (pLink->m_number_of_plans >= 1 || pLink->m_bSignalizedArterialType == true && g_SignalRepresentationFlag != signal_model_continuous_flow)
			{

				if (pLink->m_GreenEndTime_In_Second == 0)
				{
				...
				// mode 2: input_timing_status.csv
				// given: for the given link, current time -> second, to simulation_time_interval_in_second,
				// to find: PerHourCapacityAtCurrentSimulatioInterval
				if (g_SignalRepresentationFlag == signal_model_timing_status)
				{
					PerHourCapacityAtCurrentSimulatioInterval = pLink->GetTimeDependentCapacityAtSignalizedIntersectionFromTimingStatus(CurrentTime);
					if (pLink->m_FromNodeNumber == 3336 && pLink->m_ToNodeNumber == 3335 && CurrentTime >= 20)
					{
						TRACE("time = %f: link capacity %f\n", CurrentTime,PerHourCapacityAtCurrentSimulatioInterval);
					}
				}
				else
				{
					// mode 1: input_signal.csv
					PerHourCapacityAtCurrentSimulatioInterval = GetTimeDependentCapacityAtSignalizedIntersection(
						CurrentTime,
						pLink->m_current_plan_no,
						pLink->m_plan_startime_in_sec,
						pLink->m_plan_endtime_in_sec,
						pLink->m_to_node_cycle_in_sec,
						pLink->m_to_node_offset_in_sec,
						pLink->m_SaturationFlowRate_In_vhc_per_hour_per_lane,
						pLink->m_GreenStartTime_In_Second,
						pLink->m_GreenEndTime_In_Second);
				...
				if (pLink->m_LeftTurn_NumberOfLanes > 0) // with left_turn 
				{
					...
					if (g_SignalRepresentationFlag == signal_model_timing_status)
					{
						PerHourLeftTurnCapacityAtCurrentSimulatioInterval = pLink->GetTimeDependentLeftTurnCapacityAtSignalizedIntersectionFromTimingStatus(CurrentTime);
					}
					else
					{
						PerHourLeftTurnCapacityAtCurrentSimulatioInterval =
						GetTimeDependentCapacityAtSignalizedIntersection(
							...
							pLink->m_to_node_cycle_in_sec,
							pLink->m_to_node_offset_in_sec,
							pLink->m_LeftTurn_SaturationFlowRate_In_vhc_per_hour_per_lane,
							pLink->m_LeftTurnGreenStartTime_In_Second,
							pLink->m_LeftTurnGreenEndTime_In_Second);  /*left turn handling for saturation flow rate*/
					}
			...
			if (TrafficFlowModelFlag == trm_car_following)  // special condition for car following model
			{
				if (pLink->m_Cell_Prev[0] >= 0)  // used;
					pLink->LinkInCapacity = 0;
				else  // not used
				{
					pLink->LinkInCapacity = 1;
				}
			}
			...
			if (pLink->m_bMergeFlag >= 1)
			{
				...
				if (TotalInFlowCount > pLink->LinkInCapacity)  // total demand > supply
				{
					...
					if (pLink->m_bMergeFlag == 1 || pLink->m_bMergeFlag == 2)  // merge with mulitple freeway/onramp links only, or for freeway/on-ramp merge when not using freeval model
					{
						...
						if (pLink->MergeIncomingLinkVector.size() == 3) // three incoming links
						{
							TRACE("three links merge into one!!");
						}
						...
						// edited by jun
						if (pLink->MergeIncomingLinkVector.size() >= 3) // three or more incoming links
						{
							for (il = 0; il < pLink->MergeIncomingLinkVector.size(); il++)
							{
								int demand_on_other_link = 0;
								for (int ilo = 0; ilo < pLink->MergeIncomingLinkVector.size(); ilo++)
								{
									if (ilo != il) {
										demand_on_other_link += g_LinkVector[pLink->MergeIncomingLinkVector[ilo].m_LinkNo]->ExitQueue.size();
									}
								}
								pLink->MergeIncomingLinkVector[il].m_LinkInRemainingCapaityPerSimuInterval =
									pLink->LinkInCapacity - demand_on_other_link;
	...
	for (int node = 0; node < node_size; node++)
	{
		...
		while (incoming_link_count < IncomingLinkSize)
		{
			...
			while ((pLink->ExitQueue.size() > 0 && vehicle_out_count > 0) || (pLink->LeftExit_Queue.size() > 0 && vehicle_left_out_count > 0))  // either condition 1 or condition 2 is met
			{
				...
				if (g_VehicleMap[vehicle_id]->m_CurrentPassengerList.size() > 0)   // with passenger in the car
				{
					...
					if (p_Nextlink->LinkInCapacity > 0) // if there is available spatial capacity on next link, then move to next link, otherwise, stay on the current link
					{
						...
						else
						{
							...
							else  // non transit vehicle
							{
								...
								p_Nextlink->UpdateCellVehiclePosition(meso_simulation_time_interval_no,0, vi.veh_id);  // virutally move in to the first cell of the next link
						...
						if (FetchVehicleFromLeftTurnQueue == false)
						{
							...
							pLink->UpdateCellVehiclePosition(meso_simulation_time_interval_no,pLink->m_Cell.size() - 1, -1);
...
#pragma omp critical
				...
				else
				{
					...
					g_VehicleMap[vehicle_id]->m_bComplete = true;  // arrive at the destination and complete the trip

					if (g_VehicleMap[vehicle_id]->m_following_agent_id >= 0)  // activity based model to featch the next trip along the trip chain
					{
...
#pragma omp critical
					...
					pLink->ExitQueue.pop_front();
					pLink->UpdateCellVehiclePosition(meso_simulation_time_interval_no,pLink->m_Cell.size() - 1, -1);  // virutally move out from the last cell
...
void DTALink::UpdateCellVehiclePosition(int current_time_interval, int cell_no, int vehicle_id)
{
	if (cell_no >= 0 && cell_no < m_Cell.size())
	{
		m_Cell[cell_no] = vehicle_id;
	}

	if (vehicle_id >= 0)
	{
		DTAVehicle* pVehicle = g_VehicleMap[vehicle_id];
		CarFollowingData element;
		element.LinkNo = this->m_LinkNo;
		element.CellNo = cell_no;
		pVehicle->m_CarFollowingDataVector.push_back(element);


	}
	TRACE("current time interval %d; link %d->%d, cell %d, vehicle %d\n", 
		current_time_interval, 
		m_FromNodeNumber, m_ToNodeNumber,
		cell_no,
		vehicle_id);
	//TRACE
}