...
int g_SimulationDuration = 0; // the total simulation time
...
void g_ReadInputFiles()
{
	...
	if (parser_link.OpenCSVFile(InputLinkFileName))
	{
		...
		while (parser_link.ReadRecord())
		{
			...
			int number_of_leftturn_lanes = 0;
			...
			if (!parser_link.GetValueByFieldName("number_of_leftturn_lanes", number_of_leftturn_lanes))
			...
			if (g_TrafficFlowModelFlag == tfm_BPR)
			{
				...
				parser_link.GetValueByFieldName("BPR_distance_term", BPR_Distance);
			...
			for (int link_code = link_code_start; link_code <= link_code_end; link_code++)
			{
				...
				pLink->m_LeftTurn_NumberOfLanes = number_of_leftturn_lanes;
	...
	cout << "  reading movement input  "  << endl;
...
{
	// jun added
	// ReadMovementScenarioFile("input_movement.csv", scenario_no);
...
void g_DetermineDemandLoadingPeriod()
{
	...
	g_SimulationDuration = max(0, g_DemandLoadingEndTimeInMin - g_DemandLoadingStartTimeInMin);
	if (g_PlanningHorizon < g_DemandLoadingEndTimeInMin + 300)
	{
		//reset simulation horizon to make sure it is longer than the demand loading horizon
		g_PlanningHorizon = g_DemandLoadingEndTimeInMin + 300;
...
void g_ReadAMSMovementData()
{
	...
	if (parser_movement.OpenCSVFile("input_movement.csv", false))  // not required
	{
		...
		while (parser_movement.ReadRecord())
		{
			...
			// jun added: 11/28/2018
			DTANodeMovement node_movement;
			string movement_id = GetMovementStringID(up_node_id, node_id, dest_node_id);
			node_movement.in_link_from_node_id = up_node_id;
			node_movement.in_link_to_node_id = node_id;
			node_movement.out_link_to_node_id = dest_node_id;
			node_movement.turning_direction = turn_direction;

			g_NodeVector[g_NodeNametoIDMap[node_id]].m_MovementMap[movement_id] = node_movement;

			// end jun added 11/28/2018
			...
			if (g_LinkMap.find(strid) != g_LinkMap.end())
			{
				...
				if (turn_type == "Left" || turn_direction == "left")
				{
					g_LinkMap[strid]->m_LeftTurn_DestNodeNumber = dest_node_id;
				}
...
void g_ReadSignalData()
{
	CCSVParser parser_signal;


	if (parser_signal.OpenCSVFile("input_signal.csv", false))  // not required
	{
		...
		while (parser_signal.ReadRecord())
		{
			...
			parser_signal.GetValueByFieldName("from_node_id", from_node_id);
			parser_signal.GetValueByFieldName("to_node_id", to_node_id);


			string strid = GetLinkStringID(from_node_id, to_node_id);
			if (g_LinkMap.find(strid) != g_LinkMap.end())
			{

				parser_signal.GetValueByFieldName("plan_no", plan_no);


				if (plan_no >= 0 && plan_no < 20)
				{
					

					DTALink* pLink = g_LinkMap[strid];
					pLink->m_current_plan_no = plan_no;  // keep the last valid plan no
					parser_signal.GetValueByFieldName("plan_startime_in_sec", pLink->m_plan_startime_in_sec[plan_no]);
					parser_signal.GetValueByFieldName("plan_endtime_in_sec", pLink->m_plan_endtime_in_sec[plan_no]);
					parser_signal.GetValueByFieldName("to_node_cycle_in_sec", pLink->m_to_node_cycle_in_sec[plan_no]);
					parser_signal.GetValueByFieldName("to_node_offset_in_sec", pLink->m_to_node_offset_in_sec[plan_no]);

					parser_signal.GetValueByFieldName("through_green_start_time_in_sec", pLink->m_GreenStartTime_In_Second[plan_no]);
					parser_signal.GetValueByFieldName("through_green_end_time_in_sec", pLink->m_GreenEndTime_In_Second[plan_no]);
					parser_signal.GetValueByFieldName("through_saturation_flow_rate_per_hour_per_lane", pLink->m_SaturationFlowRate_In_vhc_per_hour_per_lane[plan_no]);


					parser_signal.GetValueByFieldName("left_saturation_flow_rate_per_hour_per_lane", pLink->m_LeftTurn_SaturationFlowRate_In_vhc_per_hour_per_lane[plan_no]);
					parser_signal.GetValueByFieldName("left_green_start_time_in_sec", pLink->m_LeftTurnGreenStartTime_In_Second[plan_no]);
					parser_signal.GetValueByFieldName("left_green_end_time_in_sec", pLink->m_LeftTurnGreenEndTime_In_Second[plan_no]);
					g_LinkMap[strid]->m_number_of_plans++;
				}
